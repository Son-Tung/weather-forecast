...
function Header({ city, setCity, setWeather, setWeather5day, setGeoData }: any) {
  ...
  const handleCityChange = (input: string) => {
    ...
    setTempInput(input) // cập nhật city với giá trị người dùng nhập
    try {
      if (input.length > 0) {
        const filtered = lstCities.filter((tempInput) => tempInput.name.toLowerCase().includes(input.toLowerCase())) //tìm các thành phố từ 1stcities khớp với input và lưu vào filteredcities
  ...
  const handleCityClick = (city: string) => {
    ...
    setTempInput('') // xoá giá trị hiện tại trong ô tìm kiếm
